...
simulateDIC = function(locus, sim_chrom_number, effect_size){
  ...
  marker_column_A = data.frame(select(necropsy_data, matches(paste0("_A", locus, "$"))))[,1]
  marker_column_D = data.frame(select(necropsy_data, matches(paste0("_D", locus, "$"))))[,1]
  sim_data = tbl_df(as.data.frame(llply(select(necropsy_data, necropsy1:necropsy7),
                                        makeSimData, marker_column_A, effect_size))) %>%
    mutate(A = marker_column_A, D = marker_column_D, FAMILY = necropsy_phen_std$FAMILY)
  ...
  G_mcmc = apply(array(null_model$VCV[,1:(num_necropsy_traits*num_necropsy_traits)], dim = c(n_mc, num_necropsy_traits, num_necropsy_traits)), 2:3, median)
  R_mcmc = apply(array(null_model$VCV[,-c(1:(num_necropsy_traits*num_necropsy_traits))], dim = c(n_mc, num_necropsy_traits, num_necropsy_traits)), 2:3, median)
  start <- list(R = list(V = R_mcmc), G = list(G1 = G_mcmc), liab = matrix(null_model$Liab[1,], ncol = num_necropsy_traits))
...
write_csv(DIC_power, "./data/necropsy traits/power_analysis.csv")
