...
runIntervalMCMCModel <- function(marker_term, null_formula, start = NULL, ...){
    ...
    necropsy_MCMC_flanking = MCMCglmm(as.formula(flanking_formula),
    ...
    prior = list(R = list(V = diag(num_necropsy_traits), n = 0.002),
                 G = list(G1 = list(V = diag(num_necropsy_traits) * 0.02, n = num_necropsy_traits+1)))
    necropsy_MCMC_focal = MCMCglmm(as.formula(genotype.formula),
                                random = ~us(trait):FAMILY,
                                data = as.data.frame(necropsy_data),
                                rcov = ~us(trait):units,
                                family = rep("gaussian", num_necropsy_traits),
                                ...
                                verbose = FALSE,
                                ...)
    return(list(flanking = necropsy_MCMC_flanking,
                focal    = necropsy_MCMC_focal))
}
...
all_effectsInterval_mcmc = ldply(intervalMapping_MCMC,
      function(model_pair){
           ...
           ad = effects[1:num_necropsy_traits,]
           colnames(ad) = paste("ad", colnames(ad), sep = "_")
           dm = effects[(num_necropsy_traits+1):(num_necropsy_traits+num_necropsy_traits),]
           colnames(dm) = paste("dm", colnames(dm), sep = "_")
           Sol = focal$Sol[,(neff-13):neff]
           p_ad = colSums((Sol[, 1:num_necropsy_traits]) > 0)/nrow(Sol)
           p_dm = colSums((Sol[,(num_necropsy_traits+1):(num_necropsy_traits+num_necropsy_traits)]) > 0)/nrow(Sol)
      ...
      }, .id = NULL, .parallel = TRUE) %>% tbl_df %>% mutate(count = rep(seq(intervalMapping_MCMC), each = num_necropsy_traits)) %>% select(count, everything())
effect_file = paste0("./data/necropsy traits/necropsy_effectsInterval_", flank_dist, "cM_mcmc.csv")
write_csv(all_effectsInterval_mcmc, effect_file)
dmSend(paste("Finished necropsy interval mapping with flanking", flank_dist, "cM"), "diogro")
