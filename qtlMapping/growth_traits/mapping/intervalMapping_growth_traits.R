...
runIntervalMCMCModel <- function(marker_term, null_formula, start = NULL, ...){
    ...
    growth_MCMC_flanking = MCMCglmm(as.formula(flanking_formula),
                                random = ~us(trait):FAMILY,
                                data = as.data.frame(growth_data),
                                rcov = ~us(trait):units,
                                family = rep("gaussian", num_growth_traits),
    ...
    prior = list(R = list(V = diag(num_growth_traits), n = 0.002),
                 G = list(G1 = list(V = diag(num_growth_traits) * 0.02, n = num_growth_traits+1)))
    growth_MCMC_focal = MCMCglmm(as.formula(genotype.formula),
                                random = ~us(trait):FAMILY,
                                data = as.data.frame(growth_data),
                                rcov = ~us(trait):units,
                                family = rep("gaussian", num_growth_traits),
                                ...
                                verbose = FALSE,
                                ...)
    return(list(flanking = growth_MCMC_flanking,
                focal    = growth_MCMC_focal))
}
...
all_effectsInterval_mcmc = ldply(intervalMapping_MCMC,
      function(model_pair){
           ...
           ad = effects[1:num_growth_traits,]
           colnames(ad) = paste("ad", colnames(ad), sep = "_")
           dm = effects[(num_growth_traits+1):(num_growth_traits+num_growth_traits),]
           colnames(dm) = paste("dm", colnames(dm), sep = "_")
           Sol = focal$Sol[,(neff-13):neff]
           p_ad = colSums((Sol[, 1:num_growth_traits]) > 0)/nrow(Sol)
           p_dm = colSums((Sol[,(num_growth_traits+1):(num_growth_traits+num_growth_traits)]) > 0)/nrow(Sol)
