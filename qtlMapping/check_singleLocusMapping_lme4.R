...
lme4_loci_sig = ldply(all_loci, function(x) -log(x$p.value))$V1
...
p_aSE = ggplot(data.frame(sas = sas_effects$AdditiveSE, lme4 = all_effects$ad_se),
       aes(sas, lme4)) + geom_point() + geom_abline() + ggtitle("Additive Standard Errors")
p_dSE = ggplot(data.frame(sas = sas_effects$DominanceSE, lme4 = all_effects$dm_se),
       aes(sas, lme4)) + geom_point() + geom_abline() + ggtitle("Dominance Standard Errors")
...
locus_sig = gather(data.frame(locus = sas_loci_test$Count, sas = sas_loci_test$mLOD, lme4 = lme4_loci_sig), variable, value, sas:lme4)
