...
K = 2
m0 = 10
ad = cbind(matrix(rnorm(K*m0, 0.2, 0.01), K, m0), matrix(0, K, J-m0))
dm = cbind(matrix(rnorm(K*m0, 0.1, 0.01), K, m0), matrix(0, K, J-m0))
...
families = sample(1:20, N, replace = TRUE)
beta_family = rmvnorm(20, sigma = matrix(c(0.8, 0.5, 0.5, 0.8), 2, 2))
...
estimates = rbind(data.frame(coef = 1:100, beta = beta_ad[1,], true = ad[1,], dim = 1),
                  data.frame(coef = 1:100, beta = beta_ad[2,], true = ad[2,], dim = 2)) %>%
