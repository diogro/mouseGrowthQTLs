...
markerCov = function(marker1, marker2){
  marker1_col = necropsy_markers[,makeMarkerList(marker1)]
  marker2_col = necropsy_markers[,makeMarkerList(marker2)]
...
a_biplot = PCbiplot(prcomp(a_effect_matrix[,necropsy_traits]), ids = a_effect_matrix$id)
d_biplot = PCbiplot(prcomp(d_effect_matrix[,necropsy_traits]), ids = a_effect_matrix$id)
ad_biplot = plot_grid(a_biplot, d_biplot, labels = c("C", "D"))

a_biplot_HC = PCbiplot(prcomp(a_effect_matrix_HC[,necropsy_traits]), ids = a_effect_matrix_HC$id)
d_biplot_HC = PCbiplot(prcomp(d_effect_matrix_HC[,necropsy_traits]), ids = d_effect_matrix_HC$id)
...
d_z = LG - SM
load("./Rdatas/necropsy_CovMatrices.Rdata")
necropsy_sds = apply(necropsy_phen[,necropsy_traits], 2, sd)
G = G_stan #* outer(necropsy_sds, necropsy_sds)
...
mean_a = colMeans(a_effect_matrix[,necropsy_traits])
mean_d = colMeans(d_effect_matrix[,necropsy_traits])
...
save(w_ad, w_dm, effect_matrix_additive, effect_matrix_dominance, file = "Rdatas/necropsy_add_dom_effectsMatrix.Rdata")
load(file = "Rdatas/necropsy_add_dom_effectsMatrix.Rdata")
...
save(Va,Va_pleio, Vd, Vd_pleio, file = paste0(Rdatas_folder, "necropsy_VaVd_QTL.Rdata"))
load(paste0(Rdatas_folder, "necropsy_VaVd_QTL.Rdata"))
...
png("./data/necropsy_family_Vg_FullSibG_correlation.png", width = 1500, height = 1500)
old.par = par()
...
png("./data/necropsy_Va_PleiotropyLD_and_OnlyPleitropy.png", width = 1500, height = 800)
...
g_predict = function() {plot(lt(G)~lt(Vg_mean), pch = 19, 
     ...
     main = "", xlim = c(-0., 0.002), ylim = c(-0., 0.002))
...
las =2
padj = -10.5
png("data/necropsy_cov_prediction_composite.png", width = 900, height = 900)
...
dev.off()
# png("./data/necropsy_dam_qtl_cov.png", width = 1500, height = 800)
...
#      main = "necropsy traits", xlim = c(-0.03, 0.17), ylim = c(-0.22, 0.6))
...
Vd_GP = laply(seq_along(1:400), function(i) colSums(laply(1:nrow(significantMarkerMatrix),
                                                          ...
                                                          markerMatrix)),
              .parallel = TRUE)
save(Va_GP, Vd_GP, file = paste0(Rdatas_folder, "necropsy_VaVd_GP_QTL.Rdata"))
load(paste0(Rdatas_folder, "necropsy_VaVd_GP_QTL.Rdata"))
...
g_predict_GP = function() {
  plot(lt(G)~lt(Vg_GP_mean), pch = 19, 
                             ylab = "Mixed model\nG-matrix", xlab = "Genetic covariances predicted\n from pleiotropic vectors", 
                             main = "", xlim = c(0., 0.1), ylim = c(0., 0.1))
...
las =2
padj = -10.5
png("data/necropsy_cov_prediction_composite_GP.png", width = 900, height = 900)
layout(matrix(c(1,2,3, 4), 2, 2, byrow = TRUE))
...
crss = data.frame(beta = apply(a_effect_matrix[,necropsy_traits], 1, vectorCor, beta),
                    dz = apply(a_effect_matrix[,necropsy_traits], 1, vectorCor,  d_z)) %>% gather(class, value, beta:dz)

calcPleitropicPartition = function(effect_matrix, normalize = FALSE){
  pleiotropic_partition = effect_matrix
  pleiotropic_partition[necropsy_traits] = (pleiotropic_partition[necropsy_traits]^2)
  ...
  if(normalize) pleiotropic_partition[,necropsy_traits] = pleiotropic_partition[,necropsy_traits]/rowSums(pleiotropic_partition[,necropsy_traits])
...
a_effect_matrix %>% 
gather(trait, value, necropsy_traits) %>%
  ggplot(aes(value)) + geom_histogram() + 
  facet_wrap(~trait, scales = "free_x")

x = calcPleitropicPartition(a_effect_matrix, normalize = TRUE)
mask = as.matrix(x[,necropsy_traits]) > 1/6
effect_m = as.matrix(a_effect_matrix[,necropsy_traits])
effect_m[!mask] = 0
a_effects_sig = a_effect_matrix
a_effects_sig[,necropsy_traits] = effect_m
a_effects_sig$norm = apply(as.matrix(a_effect_matrix[,necropsy_traits]), 1, Norm) 
a_effects_sig %>%
  #filter(norm > 0.1) %>%
  arrange(desc(norm)) %>%
  print(n = nrow(.))
...
a_corrs = data.frame(marker = 1:32, 
                     betaCorr = abs(apply(a_effect_matrix[,necropsy_traits], 1, vectorCor, beta)),
                     dzCorr = abs(apply(a_effect_matrix[,necropsy_traits], 1, vectorCor, d_z)),
                     norm = apply(a_effect_matrix[,necropsy_traits], 1, Norm))
d_corrs = data.frame(marker = 1:32,
                     betaCorr = abs(apply(d_effect_matrix[,necropsy_traits], 1, vectorCor, beta)),
                     dzCorr = abs(apply(d_effect_matrix[,necropsy_traits], 1, vectorCor, d_z)),
                     norm = apply(d_effect_matrix[,necropsy_traits], 1, Norm))
write.csv(a_corrs, "./data/necropsy_additive_correlations_beta_dZ.csv")
write.csv(d_corrs, "./data/necropsy_dominance_correlations_beta_dZ.csv")
...
save_plot("data/necropsy_effect_aligment_regressions.png", regressions, base_height = 4, base_aspect_ratio = 2, ncol = 2, nrow = 2)
...
necropsy_observed_plot = ggplot() + scale_x_discrete(labels = paste("Week", 1:7)) + labs(y = "Weekly necropsy (g)", x = "Start week") + geom_line(size = 1, data = filter(necropsy_prediction, Type == "Observed"), aes(trait, value, group = Line, color = Line)) + theme_cowplot()
save_plot("data/necropsy_LG_SM_F3.png", necropsy_observed_plot, base_height = 7, base_aspect_ratio = 2)


par(mar = c(0, 0, 0, 0))
g_plot = ~corrplot.mixed(cov2cor(G),       upper = "ellipse", mar = c(0, 0, 0, 0))
necropsy_curves_cov = plot_grid(necropsy_observed_plot, g_plot, labels = LETTERS[1:2], ncol = 2)
save_plot("data/necropsy_LG_SM_F3_covF3.png", necropsy_curves_cov, base_height = 6, base_aspect_ratio = 1.5, ncol = 2)

necropsy_observed_parentals_plot = ggplot() + scale_x_discrete(labels = paste("Week", 1:7)) + labs(y = "Weekly necropsy (g)", x = "Start week") + geom_line(size = 1, data = filter(necropsy_prediction, Type == "Observed", Line != "F3"), aes(trait, value, group = Line, color = Line)) + scale_color_manual(values=c("#00BA38", "#619CFF")) + theme_cowplot()
save_plot("data/necropsy_LG_SM.png", necropsy_observed_parentals_plot, base_height = 7, base_aspect_ratio = 2)

necropsy_observed_parentals_Dz_plot = ggplot() + scale_x_discrete(labels = paste("Week", 1:7)) + labs(y = "Weekly necropsy (g)", x = "Start week") + geom_line(size = 1, data = filter(necropsy_prediction, Type == "Observed", Line != "F3"), aes(trait, value, group = Line, color = Line)) + scale_color_manual(values=c("#00BA38", "#619CFF")) + 
  annotate("segment", x = 1:7, xend = 1:7, y = SM, yend = LG) + 
  annotate("text", x = 3.5, y = 6, label = "Phenotypic\ndivergence") + theme_cowplot()
save_plot("data/necropsy_LG_SM_DZ1.png", necropsy_observed_parentals_Dz_plot, base_height = 7, base_aspect_ratio = 2)

necropsy_observed_parentals_Dz2_plot = ggplot() + scale_x_discrete(labels = paste("Week", 1:7)) + labs(y = "Weekly necropsy (g)", x = "Start week") + geom_line(size = 1, data = filter(necropsy_prediction, Type == "Observed", Line != "F3"), aes(trait, value, group = Line, color = Line)) + scale_color_manual(values=c("#00BA38", "#619CFF")) +
  geom_line(data = data.frame(trait = as.factor(necropsy_traits), dz = d_z), aes(trait, dz, group = 1)) +
  annotate("text", x = 3.5, y = 5.5, label = "Phenotypic\ndivergence")
save_plot("data/necropsy_LG_SM_DZ2.png", necropsy_observed_parentals_Dz2_plot, base_height = 7, base_aspect_ratio = 2)
...
necropsy_prediction = reshape2::melt(data.frame(trait = as.factor(necropsy_traits), 
                                                ...
                                                F3_Observed = F3,
                                                LG_Observed = LG)) %>% separate(variable, c("Line", "Type"))
necropsy_pred_plot_SUR = ggplot() +
  scale_x_discrete(labels = paste("necropsy", 1:7)) +
  labs(y = "necropsy (mm2)", x = "Start necropsy") +
  geom_line(data = post, color = "gray", size = 0.5,
            linetype = 1, alpha = 0.1, aes(trait, value, group = interaction(Type, Line, iterations))) +
  geom_line(size = 1, data = necropsy_prediction,
...
(necropsy_pred_plot_HC_full = ggplot() +
  scale_x_discrete(labels = paste("necropsy", 1:7)) +
  labs(y = "necropsy (mm2)", x = "Start necropsy") +
  geom_line(data = post, color = "gray", size = 0.5,
            linetype = 1, alpha = 0.1, aes(trait, value, group = interaction(Type, Line, iterations))) +
...
predictions = plot_grid(necropsy_pred_plot_SUR, necropsy_pred_plot_HC_full, labels = LETTERS[1:2])
save_plot("data/necropsy_LG_SM_F3_predictions.png", predictions, base_height = 6, base_aspect_ratio = 1, ncol = 2)
...
a_pleiotropic_partition = calcPleitropicPartition(a_effect_matrix)
  ...
  ggplot() + 
  geom_bar(data = gather(a_pleiotropic_partition, key, value, necropsy_traits), 
...
d_pleiotropic_partition_plot =  
  ggplot() + 
  geom_bar(data = gather(d_pleiotropic_partition, key, value, necropsy_traits), aes(id, value, group = key, color = key, fill = key), stat = "identity") +
...
save_plot("data/necropsy_pleiotropic_partition_ad_dm.png", pleiotropic_Effects_ad_dm, base_height = 4.5, base_aspect_ratio = 2.5, nrow = 3) 
...
pleiotropic_partition[necropsy_traits] = sqrt(pleiotropic_partition[necropsy_traits]^2)
pleiotropic_partition$id = factor(pleiotropic_partition$id, levels = pleiotropic_partition$id)
...
markerMatrix$id = 1:353
significantMarkerPos = inner_join(significantMarkerMatrix, markerMatrix, by = c("chrom", "marker"))
significantMarkerPos$size_ad = rowSums(a_pleiotropic_partition[significantMarkerPos$id, necropsy_traits])
...
a_pleiotropic_partition_plot =  
  ...
  scale_x_discrete(breaks = a_pleiotropic_partition[markerMatrix[markerMatrix$marker==1,"id"],"id"],
                   labels = 1:19)
#save_plot("data/necropsy_pleiotropic_partition_additive.png", pleiotropic_partition_plot, base_height = 7, base_aspect_ratio = 2) 
...
significantMarkerPos$size_dm = rowSums(d_pleiotropic_partition[significantMarkerPos$id, necropsy_traits])
...
save_plot("data/necropsy_pleiotropic_partition_ad_dm_GP.png", pleiotropic_Effects_ad_dm, base_height = 4.5, base_aspect_ratio = 2.5, nrow = 3) 
#save_plot("data/necropsy_pleiotropic_partition_dominance.png", pleiotropic_partition_plot, base_height = 7, base_aspect_ratio = 2) 
...
save_plot("./data/necropsy_per_marker_additive_dominance_vectors_QTL.png", effectsStan_plot, base_height = 3, base_aspect_ratio = 1.3, 
          ncol = 4, nrow = 5)
...
a_effects_QTL_m = 
  gather(a_effect_matrix, trait, value, necropsy_traits) %>% 
  mutate(type = "QTL") %>%
  arrange(id, trait)
a_effects_GP_m = 
  gather(a_effect_matrix_HC, trait, value, necropsy_traits) %>% 
  mutate(type = "GP") %>%
...
hist(laply(1:32, function(i) abs(vectorCor(as.numeric(a_effect_matrix[i,necropsy_traits]), 
                                       as.numeric(d_effect_matrix[i,necropsy_traits]))[1])), breaks = 30,
