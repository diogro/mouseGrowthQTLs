...
colnames(coef_mean) = colnames(coef_sd) = growth_traits
...
effects = data.frame(coef_mean) %>%
           mutate(id = ID) %>%
           gather(trait, mean, growth12:growth78)
effects$class = c("additive", "dominance")
effects$upper = (data.frame(coef_upper) %>% mutate(id = ID) %>% gather(trait, upper, growth12:growth78))$upper
effects$lower = (data.frame(coef_lower) %>% mutate(id = ID) %>% gather(trait, lower, growth12:growth78))$lower
write_csv(effects, "./data/growth_significant_marker_effects.csv")
...
growth_data_wide = inner_join(growth_phen_std,
                              growth_markers,
                              by = "ID")
K        = length(growth_traits)
N        = dim(growth_data_wide)[1]
J        = nrow(significantMarkerMatrix)
n_family = length(unique(growth_data_wide$FAMILY))
family   = as.integer(as.factor(growth_data_wide$FAMILY))
ad       = as.matrix(dplyr::select(growth_data_wide, apply(significantMarkerMatrix, 1, getGenColumn, "A")))
dm       = as.matrix(dplyr::select(growth_data_wide, apply(significantMarkerMatrix, 1, getGenColumn, "D")))
y        = as.matrix(dplyr::select(growth_data_wide, growth_traits))
param_list = list(K        = K,
                  J        = J,
                  N        = N,
                  n_family = n_family,
...
write_csv(effectsStan[,names(effects)], "./data/growth_significant_marker_effects_SUR.csv")
